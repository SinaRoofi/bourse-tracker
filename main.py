...
def should_send_summary_by_time() -> bool:
    """
    فقط بعد از بسته‌شدن بازار + یه فاصله‌ی ایمنی (buffer).
    چون main.py هر ۱ دقیقه اجرا می‌شه و تا خودِ MARKET_END_TIME
    (شامل خودش) هنوز ممکنه هشدار جدید بفرسته، این تابع رو دقیقاً
    روی همون مرز صدا نمی‌زنیم - وگرنه یه اجرای daily_summary
    می‌تونه زودتر از آخرین اجرای main.py اون روز خلاصه رو بفرسته
    و بعدش هشدارهای آخر بازار جا بمونن (race condition).
    """
    from config import MARKET_END_TIME

    SUMMARY_BUFFER_MINUTES = 3  # فاصله‌ی ایمنی بعد از بسته‌شدن بازار

    end_hour, end_minute = map(int, MARKET_END_TIME.split(":"))
    cutoff_total_minutes = end_hour * 60 + end_minute + SUMMARY_BUFFER_MINUTES

    now = datetime.now(TEHRAN_TZ)
    now_total_minutes = now.hour * 60 + now.minute

    return now_total_minutes >= cutoff_total_minutes
